...
export default function Wheel({ meName, onResult }) {
  ...
  function settle() {
    ...
    // lobideki seyircilere canlı sonuç yayını (Firebase feed)
    onResult?.({ round: g.round, label: seg.l, msg: g.feed[0]?.m })
    bump()
  ...
  useEffect(() => {
    ...
    if (meName) G.current.players[0].name = String(meName).toUpperCase().slice(0, 10)
